-- schema_full.sql
-- Authoritative schema for a fresh D1 database.
-- Combines schema.sql (base) + migrations 0002-0008 in dependency order.
-- All seed data uses INSERT OR IGNORE so this file is safe to re-run.
--
-- Fresh local:  wrangler d1 execute digitaalinen_tuotepassi --local --file=./schema_full.sql
-- Fresh remote: wrangler d1 execute digitaalinen_tuotepassi --remote --file=./schema_full.sql

-- ── Tenants ───────────────────────────────────────────────────────────────────
-- Created before products so the FK reference in products is valid.
...
-- ── Products ──────────────────────────────────────────────────────────────────
-- All columns from base schema + migrations 0002/0003/0004 baked in.
...
CREATE TABLE IF NOT EXISTS products (
  ...
  published_at                TEXT,
  -- 0003: compliance workflow
  compliance_status           TEXT NOT NULL DEFAULT 'not_started',
  -- 0004: compliance engine
  category_id                 TEXT REFERENCES product_categories(id),
  target_markets_json         TEXT NOT NULL DEFAULT '["EU"]',
  created_at                  TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at                  TEXT NOT NULL DEFAULT (datetime('now'))
);
...
-- ── Product transfers (scaffold — no application logic yet) ───────────────────
...
-- ── Product documents ─────────────────────────────────────────────────────────
-- Primary store for file uploads. compliance_documents_json on products kept for legacy reads.
...
-- Per-language rows. translations_json on products kept for legacy reads.
...
-- ── Validation rules ──────────────────────────────────────────────────────────
-- Includes metadata columns from 0006_rules_metadata.sql.
...
-- ── Seed data ─────────────────────────────────────────────────────────────────
-- All inserts use INSERT OR IGNORE so this file can be re-run without error.

-- Product categories (0004 + 0007)
INSERT OR IGNORE INTO product_categories (id, code, parent_id, name_fi, name_en) VALUES
  ...
  ('cat_jewelry',     'JEWELRY',     NULL, 'Korut ja asusteet',       'Jewelry and accessories');

-- Regulations (0004 + 0007)
INSERT OR IGNORE INTO regulations (id, code, name, description, version, effective_date, status) VALUES
  (
    'reg_gpsr', 'GPSR',
    'General Product Safety Regulation',
    'EU 2023/988 — baseline safety obligations for all consumer products placed on the EU market',
    '2023/988', '2024-12-13', 'active'
  ),
  (
    'reg_epr', 'EPR',
    'Extended Producer Responsibility',
    'Repairability, reuse and end-of-life take-back obligations for electronics and batteries',
    '2024', '2024-01-01', 'active'
  ),
  (
    'reg_espr_textiles', 'ESPR_TEXTILES',
...
-- Category → regulation mappings (0004 + 0007)
INSERT OR IGNORE INTO category_regulations (id, category_id, regulation_id, market, mandatory) VALUES
  -- GPSR mandatory for all categories
  ...
  -- EPR mandatory for electronics and batteries
  ('cr_epr_ele',  'cat_electronics', 'reg_epr',           '*',  1),
  ('cr_epr_bat',  'cat_batteries',   'reg_epr',           '*',  1),
  -- ESPR Textiles draft — opt-in only
  ...
  -- TEXTILE_LABELING mandatory for textiles
  ('cr_textile_label_tex', 'cat_textiles',   'reg_textile_labeling','EU', 1),
  -- ROHS and WEEE for electronics
...
VALUES
  ...
  (
    ...
    'required_field', 'manufacturer_address', '{"always":true}', 'error',
  ...
  (
    ...
    'required_field', 'manufacturer_email', '{"always":true}', 'warning',
  ...
  (
    ...
    'required_array_min', 'safety_notes_json', '{"always":true,"min":1}', 'warning',
  ...
  (
    ...
    '{"category_codes":["ELECTRONICS"],"doc_name_pattern":".*CE.*|.*conformity.*|.*declaration.*"}',
    'info',
  ...
  (
    ...
    'required_array_min', 'materials_json', '{"category_codes":["TEXTILES"],"min":1}', 'error',
  ...
  (
    ...
    'required_array_min', 'care_instructions_json', '{"category_codes":["TEXTILES"],"min":1}', 'error',
  ...
  (
    ...
    'required_array_min', 'recycling_instructions_json', '{"category_codes":["TEXTILES"],"min":1}', 'warning',
  ...
  (
    ...
    '{"category_codes":["ELECTRONICS","BATTERIES"],"min":1}', 'warning',
...
-- Regulation rules (0007 — jewelry, textiles, electronics, batteries, furniture)
INSERT OR IGNORE INTO regulation_rules
...
VALUES
  ...
  (
    ...
    '{"category_codes":["JEWELRY"],"min":1}', 'error',
  ...
  (
    ...
    '{"category_codes":["JEWELRY"],"min":1}', 'warning',
  ...
  (
    ...
    'required_array_min', 'materials_json',
    '{"category_codes":["TEXTILES"],"min":1}', 'error',
  ...
  (
    ...
    '{"category_codes":["TEXTILES"],"min":1}', 'warning',
  ...
  (
    ...
    'required_array_min', 'materials_json',
    '{"category_codes":["TEXTILES"],"min":1}', 'info',
  ...
  (
    ...
    'required_array_min', 'substances_json',
    '{"category_codes":["ELECTRONICS"],"min":1}', 'error',
  ...
  (
    ...
    'required_array_min', 'substances_json',
    '{"category_codes":["ELECTRONICS"],"min":1}', 'warning',
  ...
  (
    ...
    'required_array_min', 'repair_instructions_json',
    '{"category_codes":["ELECTRONICS"],"min":1}', 'info',
  ...
  (
    ...
    'required_array_min', 'materials_json',
    '{"category_codes":["BATTERIES"],"min":1}', 'error',
  ...
  (
    ...
    'required_array_min', 'substances_json',
    '{"category_codes":["BATTERIES"],"min":1}', 'warning',
  ...
  (
    ...
    'required_array_min', 'materials_json',
    '{"category_codes":["FURNITURE"],"min":1}', 'error',
  ...
  (
    ...
    'required_array_min', 'substances_json',
    '{"category_codes":["FURNITURE"],"min":1}', 'warning',
  ...
  (
    ...
    'required_array_min', 'materials_json',
    '{"category_codes":["FURNITURE"],"min":1}', 'info',
...
-- Fix GPSR_CE_MARKING_INFO pattern: remove (?i) prefix invalid in V8 RegExp
-- (0005_fix_ce_pattern.sql — applied here since rules are seeded fresh above)
-- Note: the INSERT OR IGNORE above already uses the corrected pattern without (?i).
-- This UPDATE is a no-op for fresh installs but safe to run for upgrades.
